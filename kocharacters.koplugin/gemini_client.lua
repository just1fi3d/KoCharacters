...
GeminiClient.MODELS = {
    ...
    },
    {
        ...
        description  = "Free tier available (input/output free). Preview capacity limits may cause 503 errors. Paid: $0.10/$0.40 per 1M tokens, unlimited RPD.",
        input_price  = 0.100,
        output_price = 0.400,
        free_tier    = true,
        preview      = true,
    ...
    },
    {
        ...
        name         = "2.5 Flash-Lite",
        description  = "Stable GA model. Free tier available (input/output free). Paid: $0.10/$0.40 per 1M tokens, unlimited RPD. Cheapest per-token paid option.",
        input_price  = 0.100,
        output_price = 0.400,
        free_tier    = true,
        preview      = false,
    ...
    },
    {
        id           = "gemini-2.5-flash",
        name         = "2.5 Flash",
        description  = "Stable model with hybrid reasoning — much lower risk of 503 errors than any preview. Noticeably better personality synthesis and relationship nuance than Flash-Lite. Requires a billing-enabled API key.",
        input_price  = 0.150,
        output_price = 1.250,
        free_tier    = false,
        preview      = false,
    ...
    },
    {
        ...
        name         = "3.1 Pro",
        description  = "Maximum quality across all character fields. Best for complex fiction with large casts. Preview capacity limits may cause 503 errors. Paid tier only.",
        input_price  = 1.000,
        output_price = 6.000,
        free_tier    = false,
        preview      = true,
...
- Also for aliases: when the text styles a character with a fuller formal version of their name built from patronymic, toponymic, or particle elements (e.g. a character known as "Aldric" appears as "Aldric d'Maren bar Corvin", "Aldric son of Corvin", or "Aldric of York"), and the book uses such constructions as formal names — in introductions, formal address, or as a naming convention shared across characters — add that exact full form to aliases. Do not build aliases from one-off descriptive phrases that are not used as names ("the man from the harbour", "John, who grew up in the village").
...
- Do not sanitize, euphemize, or omit mature facts the text establishes. A character's sexuality, prejudices, abuse committed or suffered, disownments, and killings they order or carry out are profile-relevant facts — record them plainly in the appropriate fields ("his father had his lover executed", not "a compromising incident occurred").
...
- Never use time-relative words like "currently", "now", or "at this point" in any field — these are signs that scene state is being recorded as character trait.
- Do not sanitize, euphemize, or omit mature facts the passage establishes. A character's sexuality, prejudices, abuse committed or suffered, disownments, and killings they order or carry out are profile-relevant facts — record them plainly in the appropriate fields ("his father had his lover executed", not "a compromising incident occurred").
...
- Also for aliases: when the passage styles the character with a fuller formal version of their name built from patronymic, toponymic, or particle elements (e.g. a character known as "Aldric" appears as "Aldric d'Maren bar Corvin", "Aldric son of Corvin", or "Aldric of York"), and the book uses such constructions as formal names — in introductions, formal address, or as a naming convention shared across characters — add that exact full form to aliases. Do not build aliases from one-off descriptive phrases that are not used as names ("the man from the harbour", "John, who grew up in the village").
...
- type: validate against the allowed taxonomy — place (locations, buildings, regions), faction (organizations, groups, institutions), concept (abilities, systems, phenomena, practitioner roles — including terms that name people by an ability they possess), object (physical items or artifacts), species (beings biologically or fundamentally distinct from ordinary humans, e.g. reanimated dead, non-human creatures, fantasy races). Correct if the current value doesn't match the best-fit category.
...
- known_connections: normalize to "Name (relationship)" format. Each character or entity must appear exactly once — if the same character appears with multiple roles, merge into one entry (e.g. "Lord Vance (employer, rival)"). If the same person appears under both a short name and a full name (e.g. both "Ferron" and "Kaine Ferron"), or under a title that maps to a named character, merge into the full-name entry and drop the shorter form. Replace any role descriptors (e.g. "protagonist", "subject") with the character's actual name if it can be inferred from context.
...
- Do not sanitize or euphemize while rewriting: established facts about a character's sexuality, prejudices, abuse committed or suffered, disownments, or killings must survive cleanup stated plainly ("his father had his lover executed", not "a compromising incident occurred")
...
- Extend known_connections with any new ones found in this passage. Each character or entity must appear exactly once — if a character already listed gains a new role, update their existing entry to combine roles (e.g. "Lord Vance (employer, rival)") rather than adding a duplicate. When adding a new connection, check whether this person already appears in the list under any name form (full name, short name, or title) — if so, update that existing entry rather than adding a new one. Always use the most complete known name. Use actual character names from context, never role descriptors like "protagonist" or "subject". Format each as "Name (relationship)". Normalize any existing entries that don't follow this format.
